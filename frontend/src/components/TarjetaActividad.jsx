const TIPO_ESTILOS = {
  Taller:    { badge: 'bg-dist/10 text-dist border-dist/20',       borde: 'border-t-dist' },
  Evento:    { badge: 'bg-bogota/10 text-bogota border-bogota/20', borde: 'border-t-bogota' },
  Actividad: { badge: 'bg-sena/10 text-sena border-sena/20',       borde: 'border-t-sena' },
}
const TIPO_DEFAULT = { badge: 'bg-gray-100 text-gray-700 border-gray-200', borde: 'border-t-gray-400' }

export default function TarjetaActividad({ actividad, onClick }) {
  const { titulo, descripcion, lugar, fecha_inicio, fecha_fin, hora_inicio, hora_fin, tipo, organizador } = actividad
  ...
  function formatHora(hora) { return hora ? hora.slice(0, 5) : null }
    ...
    >
      ...
      <div className="relative z-10 p-5 flex flex-col flex-1 gap-3 pointer-events-none select-none">
        ...
        <dl className="mt-auto pt-3 border-t border-gray-100 space-y-1.5 text-xs text-gray-600">
          ...
          {horaTexto && (
            <div className="flex gap-2 items-center">
              <dt className="font-semibold text-gray-700 shrink-0 w-12">Hora</dt>
              <dd>{horaTexto}</dd>
            </div>
          )}
        </dl>
